...
def main():
    ...
    video_path = "test.mp4"
    weights_path = "runs/fall_detect/ssd/last_ssd.pth"
    output_path = "reports/ssd_result.mp4"
    model_name = "ssd"
    ...
    while cap.isOpened():
        ...
        for box, label, score in zip(boxes, labels, scores):
            if score > CONFIDENCE_THRESHOLD:
                # Возвращаем координаты рамок от 300x300 к исходному разрешению видео
                xmin = int(box[0] * (orig_w / 300.0))
                ymin = int(box[1] * (orig_h / 300.0))
                xmax = int(box[2] * (orig_w / 300.0))
                ymax = int(box[3] * (orig_h / 300.0))
