...
def main():
    ...
    num_classes = 4
    CONF_THRESHOLD = 0.6  # Порог уверенности для отображения рамок

    video_path = "test.mp4"
    weights_path = "runs/fall_detect/faster_rcnn/epoch_5.pth"
    output_path = "reports/faster_result.mp4"
    ...
    while cap.isOpened():
        ...
        if detected_fall and score >= CONF_THRESHOLD:
            tp += 1
        elif detected_fall and score <= CONF_THRESHOLD:
            fn += 1  # Пропуск кадра с падением
        elif not detected_fall and score <= CONF_THRESHOLD:
            tn += 1
        else:
